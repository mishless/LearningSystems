...
def calc_long(value):
    return abs((value * 1.5 / 0.6 - 1.5) * (value > 0.6));
...
def get_data_set(argv=None):
    ...
    parser.add_argument("-d", "--data-set", dest="data_set", help="the file containing the data set")
    ...
    with open(data_set_path, 'r') as f:
        data_set = [(float(value[0]), float(value[1]), float(value[2]), float(value[3]), value[4]) for value in [line.split(',') for line in [re.sub('\s+', ' ', line).strip() for line in f.read().splitlines()]]]
    f.closed
    return data_set

def main():
    try:
        data_set = normalize_data(get_data_set())
        result = fuzzify_and_eval(data_set, max, min)
